...
define :wls_install, :unix => true, :middleware_home_dir => nil, :java_home_dir => nil, :tmp_dir => nil, :version => nil, :os_group => nil, :os_user => nil, :source_file => nil, :template => nil, :orainst_dir => nil do
  ...
  if params[:unix]
    ...
    if ['10.3.6', '12.1.1'].include?(params[:version])
      ...
      execute 'Install WLS' do
        command "#{params[:java_home_dir]}/bin/java #{java_params} -Xmx1024m -Djava.io.tmpdir=#{params[:tmp_dir]} -jar #{params[:source_file]} -silent -responseFile #{params[:tmp_dir]}/#{params[:template]} -invPtrLoc #{params[:orainst_dir]}/oraInst.loc -ignoreSysPrereqs"
        user  params[:os_user]
        group params[:os_group]
        cwd   params[:tmp_dir]
      end
